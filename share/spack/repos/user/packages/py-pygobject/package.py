...
class PyPygobject(PythonPackage):
    ...
    def build(self, spec, prefix):
        ...
        rm('config.guess')
        rm('config.sub')
        am_spec = spec['automake']
        config_src_dir = join_path(
                am_spec.prefix,
                'share',
                'automake-%s' % am_spec.version.up_to(2))
        cp = which('cp')
        cp(join_path(config_src_dir, 'config.guess'),
           'config.guess')
        cp(join_path(config_src_dir, 'config.sub'),
           'config.sub')
