...
class Nco(AutotoolsPackage):
    ...
    def configure_args(self):
        spec = self.spec
        opts = [
            '--{0}-doc'.format('enable' if spec.satisfies('+doc') else 'disable'),
            '--{0}-mpi'.format('enable' if spec.satisfies('+mpi') else 'disable'),
            '--{0}-ncap2'.format('enable' if spec.satisfies('+ncap2') else 'disable'),
            ]
