...
class DarshanRuntime(AutotoolsPackage):
    ...
    variant(
        ...
        values=("NONE", "cobalt", "pbs", "sge", "slurm", "lsf"),
        multi=False,
    )
    ...
    def configure_args(self):
        ...
        if spec.satisfies("scheduler=slurm"):
            job_id = "SLURM_JOBID"
        if spec.satisfies("scheduler=cobalt"):
            job_id = "COBALT_JOBID"
        if spec.satisfies("scheduler=pbs"):
            job_id = "PBS_JOBID"
        if spec.satisfies("scheduler=sge"):
            job_id = "JOB_ID"
        if spec.satisfies("scheduler=lsf"):
            job_id = "LSB_JOBID"
        if job_id == "NONE":
            raise InstallError("No scheduler ID variable set.")
        ...
        if target_logpath == "default":
           ...
           logpath_option = '--with-log-path=%s' % spec.variants['logpath'].value
            ...
            raise InstallError("Target log path directory '%s' does not exist."
                    % target_logpath)
        extra_args.append(logpath_option)
        ...
        if spec.satisfies('+grouplogs'):
            ...
            extra_args.append('--disable-group-readable-logs')

        return extra_args

    def setup_run_environment(self, env):
        # default path for log file, could be user or site specific setting
        if self.spec.variants['logpath'].value == 'default':
