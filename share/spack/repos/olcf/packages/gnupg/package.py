...
class Gnupg(AutotoolsPackage):
    ...
    def configure_args(self):
        args = [
            ...
            '--disable-ldap',
            '--disable-regex',
            '--with-pinentry-pgm=%s' % self.spec['pinentry'].command.path,
            ...
            '--with-ksba-prefix=%s' % self.spec['libksba'].prefix,
            '--with-npth-prefix=%s' % self.spec['npth'].prefix,
            '--with-libiconv-prefix=%s' % self.spec['iconv'].prefix,
    ...
    def create_links(self):
        if self.spec.satisfies('@2.0:2.999'):
            bin_prefix = self.spec.prefix.bin
            ln = which('ln')
            ln('-s',
               join_path(bin_prefix, 'gpg'),
               join_path(bin_prefix, 'gpg2'))
