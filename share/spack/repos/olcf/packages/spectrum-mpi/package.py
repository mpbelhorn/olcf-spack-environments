...
class SpectrumMpi(Package):
    ...
    def setup_dependent_build_environment(self, env, dependent_spec):
        if '%xl' in dependent_spec or '%xl_r' in dependent_spec:
            env.set('MPICC',  join_path(self.prefix.bin, 'mpixlc'))
            env.set('MPICXX', join_path(self.prefix.bin, 'mpixlC'))
            env.set('MPIF77', join_path(self.prefix.bin, 'mpixlf'))
            env.set('MPIF90', join_path(self.prefix.bin, 'mpixlf'))
        else:
        ...
        env.set('OMPI_DIR', self.prefix)
        env.set('MPI_DIR', self.prefix)
