...
class Papi(Package):
    ...
    version('5.7.0', '0e7468d61c279614ff6f39488ac3600d')
    ...
    variant(
        'cpu',
        default='default',
        values=('POWER9', 'POWER8', 'default'),
        description='Target a specific CPU architecture',
        multi=False
    )
    ...
    def install(self, spec, prefix):
        with working_dir("src"):
            ...
            # Configure the CPU target, if given.
            if not self.spec.satisfies('cpu=default'):
                configure_args.append(
                    '--with-CPU={0}'.format(self.spec.variants['cpu'].value)
                    )
