...
if [[ -z "${_COMPILER_MODULE}" || -z "${_SMPI_MODULE}" ]]; then
  ...
  echo "   eg: ./$0 \"gcc/7.5.0\" [\"spectrum-mpi/default\"]"
  exit 1
fi
...
cp ./mpif-sizeof.h "${PREFIX}/include/mpif-sizeof.h"
################

################
# Install new fortran modules in their default locations
cd "${PREFIX}/lib/fortsupport_rebuild"
cp *.mod  "${PREFIX}/lib/fortsupport_${_SUFFIX}/."
...
cd "${PREFIX}/lib/fortsupport_rebuild"
cp "libmpi_ibm_usempi_ignore_tkr_custom.so"        "${PREFIX}/lib/."
cp "libmpi_ibm_usempi_ignore_tkr_custom_archive.a" "${PREFIX}/lib/."
cp "libmpi_ibm_usempif08_custom.so"                "${PREFIX}/lib/."
cp "libmpi_ibm_usempif08_custom_archive.a"         "${PREFIX}/lib/."
...
ln -s   "libmpi_ibm_usempif08_custom.so" "libmpi_ibm_usempif08_${_SUFFIX}.so"
...
ln -s   "libmpi_ibm_usempi_ignore_tkr_custom.so" "libmpi_ibm_usempi_ignore_tkr_${_SUFFIX}.so"
################
